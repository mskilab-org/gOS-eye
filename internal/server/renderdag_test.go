...
func TestRenderDAG_NodePositioning(t *testing.T) {
	...
	// Constants: nodeWidth=160, nodeHeight=64, spacingX=200, spacingY=100, paddingX=40, paddingY=40
	// Container width = 2*200 + 40*2 - (200-160) = 400 + 80 - 40 = 440
	// Container height = 2*100 + 40*2 - (100-64) = 200 + 80 - 36 = 244

	// Layer 0 has 2 nodes: layerWidth = 2*200 - 40 = 360, offset = (440 - 80 - 360)/2 = 0
	...
	// Layer 1 has 1 node: layerWidth = 1*200 - 40 = 160, offset = (440 - 80 - 160)/2 = 100
...
func TestRenderDAG_SVGEdgeBezier(t *testing.T) {
	...
	// Single layer width: nodeWidth=160, spacingX=200, paddingX=40
	// Container width = 1*200 + 80 - 40 = 240
	// Layer 0: layerWidth = 1*200 - 40 = 160, offset = (240-80-160)/2 = 0
	// A: x=40, y=40
	// B: x=40, y=140
	// Source bottom-center: (40+80, 40+64) = (120, 104)
	// Target top-center: (40+80, 140) = (120, 140)
	// cy1 = 104 + (140-104)*0.5 = 104+18 = 122
	// cy2 = 140 - (140-104)*0.5 = 140-18 = 122

	expectedPath := "M120,104 C120,122 120,122 120,140"
...
func TestRenderDAG_ContainerDimensions(t *testing.T) {
	...
	// Container: width=440, height=244
	if !strings.Contains(got, "width:440px") {
		t.Errorf("expected container width:440px, got: %s", got)
	}
...
func TestRenderDAG_NodeDimensions(t *testing.T) {
	...
	// Each node should have width:160px and height:64px
	if !strings.Contains(got, "width:160px") {
		t.Errorf("expected node width:160px, got: %s", got)
	}
...
func TestRenderDAG_SVGDimensions(t *testing.T) {
	...
	// Container: width = 1*200 + 80 - 40 = 240, height = 1*100 + 80 - 36 = 144
	// SVG should match container dimensions
	if strings.Contains(got, "<svg") {
		if !strings.Contains(got, fmt.Sprintf(`width="%d"`, 240)) {
			t.Errorf("expected SVG width=240, got: %s", got)
		}
